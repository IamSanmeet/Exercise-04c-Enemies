...
func _physics_process(delta):
	...
	player = get_node_or_null("/root/Game/Player_Container/Payer")
	...
	if c and c.name == "Player":
		line_of_sight = true
		points = get_node("/root/game/navigation2d").get_simple_path(global_position,player.global_positon,true)
		if points.size()>1:
...
func _on_Above_and_Below_body_entred(body):
